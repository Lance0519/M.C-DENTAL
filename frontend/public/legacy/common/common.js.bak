...
const Auth = {
    ...
    login(usernameOrEmail, password) {
        ...
        if (!user) {
            user = Storage.getUserByEmail(usernameOrEmail);
        }
        ...
        if (user && user.password === password) {
            ...
            if (typeof logActivity === 'function') {
                ...
                logActivity('authentication', 'User Login', `${userType} ${user.fullName} (${user.email || user.username}) logged in`, user.id);
            }
    ...
    logout() {
        ...
        // Redirect to React app login page
    ...
    // Register new patient (supports both local storage and backend API)
    async register(userData) {
                ...
                if (isBackendAvailable) {
                    try {
                        ...
                        if (response.success || response.id) {
                            ...
                            // User data will be in response, create user object
                            const newUser = {
                        ...
                        // Check if it's a validation error (username/email exists)
                        if (apiError.message && (apiError.message.includes('already exists') || apiError.message.includes('Username already') || apiError.message.includes('Email already'))) {
                            return { success: false, message: apiError.message };
                        }
                        console.warn('Backend registration failed, trying local storage:', apiError);
                    }
                }
            } catch (error) {
        ...
        const existingEmail = Storage.getUserByEmail(userData.email);
        if (existingEmail) {
        ...
        // Log patient registration activity
        if (typeof logActivity === 'function') {
            logActivity('patient', 'Patient Account Created', `New patient account created for ${newUser.fullName} (${newUser.email}) via local storage`, newUser.id);
        }
