...
const Storage = {
    ...
    init() {
        ...
        if (!existingData) {
            ...
            data.services = data.services.filter((service) => service && service.id && service.id !== 'srv019' && service.id !== 'srv020');
            // Update or add default services
            defaultData.services.forEach((defaultService) => {
                ...
                if (existing && existing.id !== 'srv019' && existing.id !== 'srv020') {
                    ...
                    data.services.push(Object.assign({}, defaultService));
                }
            });
            ...
            const promo001 = data.promos.find((p) => p.id === 'promo001');
            const promo002 = data.promos.find((p) => p.id === 'promo002');
            const defaultPromo001 = defaultData.promos.find((p) => p.id === 'promo001');
            const defaultPromo002 = defaultData.promos.find((p) => p.id === 'promo002');
            if (defaultPromo001) {
                if (promo001) {
                    Object.assign(promo001, defaultPromo001);
                }
                else {
                    data.promos.push(Object.assign({}, defaultPromo001));
                }
            }
            if (defaultPromo002) {
                if (promo002) {
                    ...
                    data.promos.push(Object.assign({}, defaultPromo002));
                }
            }
            ...
            defaultData.schedules.forEach((defaultSchedule) => {
                const existing = existingSchedulesMap.get(defaultSchedule.id);
                if (!existing) {
                    // Add missing default schedule
                    data.schedules.push(Object.assign({}, defaultSchedule));
                }
            });
    ...
    createUser(user) {
        const data = this.getData();
        const newUser = Object.assign(Object.assign({}, user), { id: 'user' + Date.now(), dateCreated: new Date().toISOString() });
        if (newUser.role === 'patient') {
            // Ensure firstName and lastName are set for patients
            const patientUser = newUser;
    ...
    createDoctor(doctor) {
        const data = this.getData();
        const newDoctor = Object.assign(Object.assign({}, doctor), { id: 'doc' + Date.now() });
    ...
    getServices() {
        ...
        services = services.map(service => {
            ...
            if (trimmedName !== service.name || trimmedDuration !== service.duration) {
                modified = true;
                return Object.assign(Object.assign({}, service), { name: trimmedName, duration: trimmedDuration });
            }
            return service;
        });
    ...
    getServiceById(id) {
        ...
        if (id === 'srv001' && service) {
            ...
            if (serviceName !== 'consultation') {
                ...
                const correctedService = Object.assign(Object.assign({}, service), { id: 'srv001', name: 'Consultation', description: service.description || 'Dental consultation - P500 only, but FREE if you avail any dental procedures', duration: service.duration || '30 mins', price: service.price || '₱500 (Free with any dental procedure)' });
    ...
    createPatient(patient) {
        ...
        const newPatient = Object.assign(Object.assign({}, patient), { id: patient.id || ('pat' + Date.now() + '_' + Math.random().toString(36).substr(2, 9)), dateCreated: patient.dateCreated || new Date().toISOString(), role: 'patient' });
    ...
    createAppointment(appointment) {
        ...
        const newAppointment = Object.assign(Object.assign({}, appointment), { date: appointmentDate, time: appointmentTime, appointmentDate: appointmentDate, appointmentTime: appointmentTime });
    ...
    createSessionImage(imageData) {
        ...
        const newImage = Object.assign({ id: 'img' + Date.now(), uploadedAt: new Date().toISOString() }, imageData);
    ...
    createSchedule(schedule) {
        const data = this.getData();
        const newSchedule = Object.assign(Object.assign({}, schedule), { id: 'sch' + Date.now() });
    ...
    createMedicalHistory(record) {
        ...
        const newRecord = Object.assign({ id: 'med' + Date.now(), createdAt: new Date().toISOString() }, record);
    ...
    createPromo(promo) {
        ...
        const newPromo = Object.assign(Object.assign({}, promo), { id: 'promo' + Date.now(), createdAt: new Date().toISOString() });
    ...
    markNotificationAsRead(notificationId) {
        var _a;
        const data = this.getData();
        const notification = (_a = data.notifications) === null || _a === void 0 ? void 0 : _a.find(n => n.id === notificationId);
        if (notification) {
